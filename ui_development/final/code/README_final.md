...
## What changed in v2
- **Colour:** muted plum primary on warm neutral surfaces, adopted from Astra's concept (`ui_development/DESIGN_codex.md`).
...
| Dictate | Start dictation and speak | **Frequency Lanes waveform** (spec: `ui_development/audio_waveform_component.md`): louder → taller pillars; low sounds lift the left, high sounds the right; silence settles to the still Quiet Pillars. Your browser asks for the microphone; if declined/unavailable it plays a labelled synthetic phrase ("simulated input") |
...
**Automated check:** `node final/code/verify_final.mjs` (from `ui_development/`) drives the flows above in headless Chromium and asserts the results (test captures go to a temp folder). It reuses the Playwright install already in `ui_development/.tools_codex`; it is the only file here that depends on anything outside this folder, and the UI itself does not need it.
