...
- **Migrations:** `feat/storage` owns the migration runner and `m001_base.py`. Each wave-2 branch owns exactly one pre-numbered migration file (002 settings, 003 dictionary, 004 history). The coordinator allocates any later number. A migration only adds its own tables, so the three wave-2 branches never edit the same file. Because the runner requires contiguous versions, the coordinator merges them in number order (002 → 003 → 004); wave-2 code imports `Connection` and `IntegrityError` from `wispr_clone.storage` instead of `sqlite3`, so the `sqlite3` impact fragment keeps one owner.
