...
- Folder rules (pipeline §7.2): `tests/integration/<feature>/` → Sol feature; `tests/integration/pipeline/` and `tests/integration/app/` → Sol integration. `tests/fixtures/audio/generated/<feature>_*` → Sol feature; `tests/fixtures/audio/speech/**` and its `SOURCES.md` → Sol boundary. Wave-2 migrations: one pre-numbered file per branch (`m002_settings`, `m003_dictionary`, `m004_history`).
