...
- Confirm the checkout and existing changes before editing. Preserve other agents' work. Keep source in the assigned worktree; do not modify the original UI prototype, research, pipeline plans, global settings, or Cognee plugin/connection configuration.
...
- GPT-6 Luna writes production implementation, production tooling, and assigned CI/packaging code. Luna may run/read tests but does not author, remove, relax, or mark tests to make its code pass.
- GPT-6 Sol writes tests, fixtures, probes, conformance suites, evaluation cases, and test infrastructure. Sol may inspect source and propose fixes but does not quietly repair production code in a test task.
...
- Sol boundary may contribute shared conformance cases only through a coordinator-assigned exclusive change. The Sol integration role remains the default shared-test owner. One writer per shared fake, fixture, probe, impact fragment, or configuration file.
