...
|---|---|
| `feat/storage` | `src/wispr_clone/storage/**` |
| `feat/settings-models` | `src/wispr_clone/settings/schema.py`, `src/wispr_clone/models/**` |
| `feat/settings-store` | `src/wispr_clone/settings/store.py` |
| `feat/dictionary-core` | `src/wispr_clone/dictionary/apply.py`, `src/wispr_clone/dictionary/import_export.py` |
| `feat/dictionary-repo` | `src/wispr_clone/dictionary/repo.py`; persistence changes to `import_export.py` only after exclusive handoff |
| `feat/history` | `src/wispr_clone/history/**` |

Storage-backed work waits for storage plus its leaf prerequisites. Do not edit tests, shared contracts, manifests, pipeline orchestration, or application handlers. Shared schema/migration changes needed by another branch require a coordinated storage task; do not make the history branch a second storage writer.
