...
| Tools missing from this shell's PATH | `command -v` found no `uv`, `gh`, or `shellcheck` | Do not imply the pipeline checks can run yet; Windows-side installation status was not inspected |
...
- Run at most two feature work orders actively at first. RED → GREEN → verification is sequential within a feature. Model labels do not imply that GPT-6 agent inference should be installed on this GPU; local GPU budgeting is for the app's Voxtral/Llama stack and verification jobs.
...
- One owner controls GPU experiments. First establish a measured configuration; the controlled G2 experiment must still measure both selected model servers together. Account for weights, runtime overhead, context/KV caches, audio buffers, and other GPU users. Do not assume a download size equals runtime VRAM use.
...
- Keep Windows and WSL Python environments separate. Python 3.12 remains the project proposal; the installed interpreter and native dependency compatibility must be checked at G1. Never share a Linux venv with Windows.
