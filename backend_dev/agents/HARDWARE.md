...
| GPU | `nvidia-smi` outside the sandbox: NVIDIA GeForce RTX 5080, driver 610.88 | GPU is accessible outside this sandbox. transcribe.cpp's CUDA 12 wheel runs Voxtral on it from Windows (CODEMAP §7 G2 record) |
| GPU memory | 16,303 MiB total. Used before any Wispr model: 7.7–13.2 GB across checks on 2026-09-24. LM Studio (Llama 3.1 8B + Cognee's embedding model) holds about 7.5 GB; Voxtral adds about 2.1 GB; the G2 run peaked at 15.1 GB | About 1 GB of headroom with everything loaded. Record free VRAM before every GPU job; total VRAM is not free VRAM |
...
- Allow one resource-heavy local job at a time: model acquisition/loading, inference evaluation, large native build, or packaged runtime exercise. Load at most one Voxtral model at a time across all worktrees, and never start a second LM Studio or LLM server.
...
- **Sol boundary is the single GPU owner.** It runs every tier G/E job; other roles request runs through the coordinator. It never loads, unloads or reconfigures LM Studio models without approval (Cognee depends on them). First establish a measured configuration; the controlled G2 experiment must still measure both selected model servers together. Account for weights, runtime overhead, context/KV caches, audio buffers, and other GPU users. Do not assume a download size equals runtime VRAM use.
...
| Windows venv on the local GPU (launched from WSL) + LM Studio | transcribe.cpp/Voxtral streaming, LM Studio cleanup, measured memory/latency, loopback-only reachability, offline inference | Interactive behavior unless separately exercised on the real desktop |
