...
|---|---|---|---|
| Local STT | Voxtral Mini 4B Realtime 2602, GGUF Q4_K_M from `handy-computer/Voxtral-Mini-4B-Realtime-2602-gguf` (2.8 GB, already in LM Studio's model folder; the app reads the file, LM Studio does not serve it) | transcribe.cpp in-process, CUDA, 16 kHz mono float32 chunks | Measured on synthetic speech (CODEMAP §7 G2 record): ~15× faster than real time warm, final text 0.04–0.24 s after audio ends, cancel 0.004 s, +2.1 GB VRAM. Live USB microphone: every content word correct, final text 0.25 s after stop. Dictionary biasing support unverified |
